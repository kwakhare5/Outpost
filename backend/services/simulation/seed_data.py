...
PRODUCTS: list[SeedProduct] = [
    # DAIRY (5 products)
    SeedProduct(_id('prod-toned-milk'), 'Toned Milk 500ml', 'dairy', 'pack', 72, 28.0, 'Bhiwandi RFC (Dairy)',
               substitution_group='milk', daily_demand_mean=18.0, daily_demand_std=4.0, weekend_multiplier=0.9),
    SeedProduct(_id('prod-full-cream-milk'), 'Full Cream Milk 1L', 'dairy', 'pack', 72, 66.0, 'Panvel RFC (Dairy)',
               substitution_group='milk', daily_demand_mean=14.0, daily_demand_std=3.5, weekend_multiplier=0.9),
    SeedProduct(_id('prod-curd'), 'Fresh Curd 400g', 'dairy', 'cup', 96, 40.0, 'Bhiwandi RFC (Dairy)',
    ...
    SeedProduct(_id('prod-butter'), 'Butter 100g', 'dairy', 'pack', 240, 56.0, 'Bhiwandi RFC (Dairy)',
               daily_demand_mean=6.0, daily_demand_std=2.0, weekend_multiplier=1.1),

    # BAKERY (5 products)
    SeedProduct(_id('prod-white-bread'), 'White Bread 400g', 'bakery', 'loaf', 48, 40.0, 'Thane RFC (Bakery)',
               substitution_group='bread', daily_demand_mean=15.0, daily_demand_std=3.5, weekend_multiplier=1.0),
    SeedProduct(_id('prod-wheat-bread'), 'Whole Wheat Bread 400g', 'bakery', 'loaf', 48, 50.0, 'Kurla RFC (Bakery)',
    ...
    SeedProduct(_id('prod-rusk'), 'Toast Rusk 200g', 'bakery', 'pack', 720, 35.0, 'Thane RFC (Bakery)',
               daily_demand_mean=5.0, daily_demand_std=1.5, weekend_multiplier=1.0),
    SeedProduct(_id('prod-cake'), 'Fruit Cake Slice', 'bakery', 'piece', 72, 45.0, 'Kurla RFC (Bakery)',
               daily_demand_mean=4.0, daily_demand_std=2.0, weekend_multiplier=1.5),

    # PRODUCE (5 products)
    SeedProduct(_id('prod-tomatoes'), 'Tomatoes 500g', 'produce', 'pack', 96, 32.0, 'Nashik RFC (Produce)',
               daily_demand_mean=16.0, daily_demand_std=4.0, weekend_multiplier=1.1),
    SeedProduct(_id('prod-onions'), 'Onions 1kg', 'produce', 'bag', 168, 40.0, 'Nashik RFC (Produce)',
    ...
    SeedProduct(_id('prod-bananas'), 'Bananas 6pc', 'produce', 'bunch', 72, 45.0, 'Vashi RFC (Produce)',
               daily_demand_mean=10.0, daily_demand_std=3.0, weekend_multiplier=1.2),
    SeedProduct(_id('prod-green-chillies'), 'Green Chillies 100g', 'produce', 'pack', 96, 15.0, 'Nashik RFC (Produce)',
               daily_demand_mean=8.0, daily_demand_std=2.5, weekend_multiplier=1.0),

    # STAPLES (5 products)
    SeedProduct(_id('prod-basmati-rice'), 'Basmati Rice 1kg', 'staples', 'bag', 4320, 120.0, 'Taloja RFC (Staples)',
    ...
    SeedProduct(_id('prod-sugar'), 'Sugar 1kg', 'staples', 'bag', 8640, 48.0, 'Taloja RFC (Staples)',
               daily_demand_mean=5.0, daily_demand_std=1.0, weekend_multiplier=1.0),
    SeedProduct(_id('prod-cooking-oil'), 'Sunflower Oil 1L', 'staples', 'bottle', 4320, 140.0, 'Taloja RFC (Staples)',
               daily_demand_mean=5.0, daily_demand_std=1.5, weekend_multiplier=1.0),
    ...
    SeedProduct(_id('prod-biscuits'), 'Marie Biscuits 300g', 'packaged', 'pack', 2160, 30.0, 'Thane RFC (Bakery)',
               daily_demand_mean=7.0, daily_demand_std=2.0, weekend_multiplier=1.1),
    SeedProduct(_id('prod-chips'), 'Potato Chips 150g', 'packaged', 'pack', 2160, 40.0, 'Taloja RFC (Staples)',
               daily_demand_mean=6.0, daily_demand_std=2.5, weekend_multiplier=1.4),
    SeedProduct(_id('prod-jam'), 'Mixed Fruit Jam 200g', 'packaged', 'jar', 4320, 95.0, 'Bhiwandi Central RFC (Packaged)',
               daily_demand_mean=3.0, daily_demand_std=1.0, weekend_multiplier=1.0),
    SeedProduct(_id('prod-ketchup'), 'Tomato Ketchup 500g', 'packaged', 'bottle', 4320, 110.0, 'Bhiwandi Central RFC (Packaged)',
               daily_demand_mean=3.0, daily_demand_std=1.0, weekend_multiplier=1.0),
]
